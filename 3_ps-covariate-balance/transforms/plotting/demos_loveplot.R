...
## Purpose: This function creates a loveplot for demographic confounders
## Date: May 2022
...
love.plot.demos <- function(model_selection, demos) {
  # restrict results to only demographic confounders
  df = model_selection
  plot_covs <- df %>%
    select(demos)
  ...
  love_plot<- love.plot(
    ...
    wrap=5, 
    size=2.5, 
    title = "Balance of Demographic Covariates",
