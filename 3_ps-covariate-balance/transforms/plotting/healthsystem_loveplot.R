...
## Purpose: This function creates a loveplot for health-system & spaciotemporal confounders
## Date: May 2022
...
love.plot.health_system <- function(model_selection, health_system_covariates) {
  df = model_selection
  
  plot_covs <- df %>%
    select(health_system_covariates)
  ...
  love_plot<- love.plot(
    ...
    wrap=5, 
    size=2.5, 
    title = "Balance of Health System Covariates",
