...
## Purpose: This function creates a loveplot for comorbidity confounders
## Date: May 2022
...
love.plot.top10conditions <- function(model_selection, top10_conditions) {
  df = model_selection
  
  plot_covs <- df %>%
    select(top10_conditions)
  ...
  love_plot<- love.plot(
    ...
    wrap=5, 
    size=2.5, 
    title = "Balance of Condition Covariates",
